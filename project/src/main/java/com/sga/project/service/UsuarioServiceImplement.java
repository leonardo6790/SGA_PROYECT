...
public class UsuarioServiceImplement implements UsuarioService {
    ...
    public UsuarioServiceImplement(UsuarioMapper usuMap, UsuarioRepositoryes usuRepo, RolRepositoryes rolRepo,
            BarrioRepositoryes barRepo, TipoDocRepositoryes tipDocRepo, PasswordEncoder passwordEncoder) {
        this.usuMap = usuMap;
    ...
    public UsuarioDto getUsuario(Integer numDoc) {
        return usuRepo.findById(numDoc).map(usuMap::toUsuarioDto).orElseThrow(
                () -> new EntityNotFoundException("Usuario no encontrado por el Numero de Documento: " + numDoc));
    }
    ...
    public List<UsuarioDto> getVendedores() {
        return usuRepo.findVendedores().stream().map(usuMap::toUsuarioDto).toList();
    }

    @Override
    public void deleteUsuario(Integer numDoc) {
        Usuario usuario = usuRepo.findById(numDoc).orElseThrow(
                () -> new EntityNotFoundException("Usuario no encontrado por el Numero de Documento: " + numDoc));
    ...
    public UsuarioDto updateUsuario(UsuarioDto usuarioDto) {
        ...
        // Solo actualizar rol y tipoDoc si se proporcionan (son opcionales en
        // actualización)
    ...
    public UsuarioDto toggleActivoUsuario(Integer numDoc, Boolean activo) {
        Usuario usuario = usuRepo.findById(numDoc)
                .orElseThrow(() -> new EntityNotFoundException(
                        "Usuario no encontrado por el Numero de Documento: " + numDoc));
